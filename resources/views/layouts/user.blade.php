...
<body class="min-h-screen flex flex-col antialiased bg-slate-50" x-data="{ mobileNavOpen: false }">
    ...
    <header class="bg-white/90 backdrop-blur-sm border-b border-slate-200/80 sticky top-0 z-30">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="hidden md:flex items-center gap-2.5">
                    <div class="text-right">
                        <p class="text-xs font-bold text-slate-900 leading-none">{{ Auth::user()->name ?? 'Peserta' }}</p>
                        <p class="text-[10px] text-slate-400 mt-0.5">{{ Auth::user()->email ?? '' }}</p>
                    </div>
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 text-white flex items-center justify-center font-black text-xs border-2 border-white shadow-sm">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-200 bg-white hover:bg-rose-50 hover:border-rose-200 text-slate-600 hover:text-rose-600 text-xs font-bold transition-all duration-150 shadow-sm">
        ...
        <div x-show="mobileNavOpen" x-cloak class="sm:hidden border-t border-slate-100 px-4 pb-3 pt-2 space-y-1 bg-white slide-up">
            ...
            <div class="pt-2 border-t border-slate-100 flex items-center gap-2.5 px-3">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-900">{{ Auth::user()->name ?? 'Peserta' }}</p>
